# Phase 4 — AI Support Router and Tuning (Weeks 52–54)
...
## Entry Conditions (must be true before Week 52 starts)
...
5. AI budget guard `aiSupport` feature key registered in W50; consumed in W52.
...
### Week 52 — AI Router, Context Injection, Escalation Pipeline
...
### Week 53 — Eval, Analytics, CSAT, Tagging
- Extend the W49 AI evaluation harness with a `support` eval set, ≥ 100 examples drawn from real W47–W52 tickets, with rubric (correctness, tone, escalation appropriateness, no-leakage). Record baseline. Wire to CI for any prompt or model-tier change.
...
### Week 54 — Hardening, Tuning, Phase 4 Close
...
|------|-----------|----------------------|
| AI provider for support (reuse W19 provider vs separate) | Before W52 | Reuse W19 / Phase 1 provider; same SDK, separate feature key |
| Confidence threshold initial value | End of W52 | 0.75 (re-tune in W54 from real data) |
| SLA for human reply after escalation | Carried over from W50 | 24 business hours v1; tighten post-launch |
| CSAT scale (1–5 vs thumbs up/down) | End of W53 | Thumbs up/down + optional comment for v1 |
| Out-of-scope topics policy (refuse vs escalate) | End of W52 | Escalate with low confidence; never refuse silently |

## Parallel Streams (run alongside Weeks 52–54)
...
2. Test deltas: router unit + integration tests with adversarial cases (W52), eval baseline recorded (W53), tuned thresholds documented (W54).
...
| Cross-tenant data leakage via prompt injection | Critical | Low | Adversarial test suite in W52; never include other-tenant data in context; KB is platform-wide only |
...
Phase 4 cards extend the prefix scheme using the `SUP` code introduced in Phase 3.5 W50:
- `[W52-AI-010]` aiSupportRouter Cloud Function
- `[W52-SEC-008]` Cross-tenant adversarial tests for support context
- `[W52-SUP-005]` AI router escalation email + SLA timer
- `[W53-RPT-006]` Support analytics dashboard
- `[W53-SUP-006]` CSAT survey + ticket tagging
- `[W54-AI-011]` Confidence threshold tuning + KB iteration
...
- Phase 3.5 (precedes this): [PHASE3_5_RELEASE_READINESS_PLAN_WEEKS_47_TO_51.md](PHASE3_5_RELEASE_READINESS_PLAN_WEEKS_47_TO_51.md)
- AI eval harness origin: W47 in [PHASE3_5_RELEASE_READINESS_PLAN_WEEKS_47_TO_51.md](PHASE3_5_RELEASE_READINESS_PLAN_WEEKS_47_TO_51.md)
