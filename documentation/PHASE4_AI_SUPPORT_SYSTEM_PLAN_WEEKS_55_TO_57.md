...
|------|-----------|----------------------|
| AI provider for support (reuse W19 provider vs separate) | Before W55 | Reuse W19 / Phase 1 provider; same SDK, separate feature key |
| Confidence threshold initial value | End of W55 | 0.75 (re-tune in W57 from real data) |
| SLA for human reply after escalation | Carried over from W50 | 24 business hours v1; tighten post-launch |
| CSAT scale (1–5 vs thumbs up/down) | End of W56 | Thumbs up/down + optional comment for v1 |
| Out-of-scope topics policy (refuse vs escalate) | End of W55 | Escalate with low confidence; never refuse silently |
...
2. Test deltas: router unit + integration tests with adversarial cases (W55), eval baseline recorded (W56), tuned thresholds documented (W57).
...
| Cross-tenant data leakage via prompt injection | Critical | Low | Adversarial test suite in W55; never include other-tenant data in context; KB is platform-wide only |
| AI deflection too low → human queue floods | High | Medium | Tune threshold in W55; add macros to speed human replies; KB iteration cycle |
...
- `[W55-AI-010]` aiSupportRouter Cloud Function
- `[W55-SEC-008]` Cross-tenant adversarial tests for support context
- `[W55-SUP-005]` AI router escalation email + SLA timer
- `[W56-RPT-006]` Support analytics dashboard
- `[W56-SUP-006]` CSAT survey + ticket tagging
- `[W57-AI-011]` Confidence threshold tuning + KB iteration
