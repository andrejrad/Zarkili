...
## Decision Gate Outcomes (resolved at Phase 2.1 close, 2026-04-29)
All Phase 2 decision gates have been resolved as of W32 close. This table is the authoritative record consumed by Phase 2.2, Phase 2.3, and Phase 3 plans. Any future re-opening of these gates requires a written re-baseline trigger.

| Gate | Outcome | Evidence / location |
|------|---------|---------------------|
| Dark mode | **No (post-launch)** — default applied | W21–W32 shipped light-only; theme tokens single-mode |
| Tablet/web breakpoints | **Web responsive only, no tablet-specific layouts** — default applied | W32 store-readiness pass; mobile-first sizing across all 60+ screens |
| Map view in Discover | **Yes (basic map + list toggle)** — built | `ExploreMapScreen` planned in W34 Stream B (Phase 2.2); pin + cluster planned per W30 spec |
| Recurring bookings | **UI YES, backend post-launch** | `RecurringBookingSheet` built in W30 Batch J.2.2; navigator gates the user-facing path with a "Coming soon" message until backend ships post-GA. Phase 3 W43 admin recurring-booking management deferred to post-launch. |
| Multi-service bookings | **YES (built)** | `MultiServiceBookingScreen` built in W30 Batch J.2.1; live in production booking flow. Group / multi-party booking remains **No (post-launch)**. |
| Comments on marketplace posts | **No (post-launch)** — default applied | Marketplace v1 read-only consumer surface; no comment composer shipped |
| Group messaging threads | **No (post-launch)** — default applied | Messaging is 1:1 client ↔ salon; group thread support deferred |
| RTL languages | **No (post-launch)** — default applied | W32 i18n shipped en-US, en-GB, de-DE, hr-HR (all LTR); Arabic/Hebrew deferred |
| iOS Live Activities | **No** — default applied | Not in store-readiness shipped scope |
| Android widgets | **No** — default applied | Not in store-readiness shipped scope |
| App Clip / Instant App | **No** — default applied | Not in store-readiness shipped scope |
| Tenant owner mobile dashboard | **Web-only, mobile read-only** — default applied | Phase 3 admin plan (W38–W49) is web-primary by default; native parity only for floor-operator flows (calendar, queue, manual booking) |

**Downstream consumers of this table:**
- [PHASE2_2_NAVIGATION_WIRING_WEEKS_33_TO_34.md](PHASE2_2_NAVIGATION_WIRING_WEEKS_33_TO_34.md) — confirms which screens are wired (recurring sheet wired with "coming soon" backend gate)
- [PHASE2_3_CONSUMER_FIREBASE_INTEGRATION_WEEKS_35_TO_37.md](PHASE2_3_CONSUMER_FIREBASE_INTEGRATION_WEEKS_35_TO_37.md) — recurring backend NOT wired (post-launch)
- [PHASE3_ADMIN_UI_PLAN_WEEKS_38_TO_49.md](PHASE3_ADMIN_UI_PLAN_WEEKS_38_TO_49.md) — W43 recurring-booking admin **descoped to post-launch**; admin UI remains **web-primary** with native parity only for floor-operator flows
